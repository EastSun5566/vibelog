...
export interface AppConfig {
  dataRoot: string; appOrigin: string; appHostname: string; previewOrigin: string;
  betterAuthSecret: string; aiUserDailyLimit: number; aiGlobalDailyLimit: number;
...
export function loadAppConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  return { dataRoot: resolve(env.DATA_ROOT ?? '.data'), appOrigin: origin.origin, appHostname: origin.hostname, previewOrigin: `${origin.protocol}//${previewHost}`, betterAuthSecret, aiUserDailyLimit: positiveInteger(env.VIBELOG_AI_USER_DAILY_LIMIT, 20, 'VIBELOG_AI_USER_DAILY_LIMIT'), aiGlobalDailyLimit: positiveInteger(env.VIBELOG_AI_GLOBAL_DAILY_LIMIT, 200, 'VIBELOG_AI_GLOBAL_DAILY_LIMIT'), aiProvider, aiModel: env.VIBELOG_AI_MODEL ?? 'gpt-4o-mini', secureCookies: origin.protocol === 'https:' };
}
