...
async function setup() {
  ...
  const config: AppConfig = { nodeEnv: 'test', dataRoot: root, appOrigin: 'http://localhost:3000', appHostname: 'localhost', previewOrigin: 'http://preview.localhost:3000', betterAuthSecret: 'a'.repeat(32), betaInviteDigest: createHash('sha256').update('invite-code-with-24-characters').digest(), aiUserDailyLimit: 20, aiGlobalDailyLimit: 200, aiProvider: 'openai', aiModel: 'gpt-4o-mini', secureCookies: false };
...
describe('hosted app boundaries', () => {
  it('requires the beta code, reserves system usernames, and creates a host-only session', async () => {
    ...
    const reserved = await app.request('http://localhost:3000/auth/register', { method: 'POST', headers: { host: 'localhost:3000', origin: 'http://localhost:3000', 'x-forwarded-for': '1.1.1.1', 'content-type': 'application/x-www-form-urlencoded' }, body: form({ inviteCode: 'invite-code-with-24-characters', username: 'preview', password: 'long-enough-password' }) });
    expect(reserved.status).toBe(400);
    const denied = await app.request('http://localhost:3000/auth/register', { method: 'POST', headers: { host: 'localhost:3000', origin: 'http://localhost:3000', 'x-forwarded-for': '2.2.2.2', 'content-type': 'application/x-www-form-urlencoded' }, body: form({ inviteCode: 'wrong-code', username: 'alice', password: 'long-enough-password' }) });
    expect(denied.status).toBe(401);
    const response = await app.request('http://localhost:3000/auth/register', { method: 'POST', headers: { host: 'localhost:3000', origin: 'http://localhost:3000', 'x-forwarded-for': '3.3.3.3', 'content-type': 'application/x-www-form-urlencoded' }, body: form({ inviteCode: 'invite-code-with-24-characters', username: 'Alice', password: 'long-enough-password' }) });
  ...
  it('serves only active immutable releases on username hosts', async () => {
    ...
    const published = await app.request('http://alice.localhost:3000/', { headers: { host: 'alice.localhost:3000' } });
